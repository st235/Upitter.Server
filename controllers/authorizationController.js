...
class AuthorizationController extends BaseController {
	...
	vkVerify(req, res, next) {
		...
		socialRequestUtils
			...
			.then(user => {
				console.log(user);
				return this.usersManager.checkSocialExistence('vk', user.response[0]);
			})
			.then(user => {
				userModel = user;
				return userModel;
	...
	twitterWebVerify(req, res, next) {
		...
		this
			...
			.then(user => authUtils.createToken(this.authorizationClient, user.customId))
			.then(accessToken => res.redirect(301, `${webDomainConfig.domain + webDomainConfig.success}?accessToken=${accessToken}`))
			.catch(next);
	}
